...
export class Portfolio {
  ...
  calculate(): {name: string; amount: Fraction}[] {
    ...
    targetDeviation = targetDeviation.add(
      this.calcDevForMoney(this.funds, affectedAssets.slice(0), new Fraction(0))
    )

    const plan = affectedAssets.map((a) => {
      return {
        name: a.name,
        amount: a.targetValue.mul(targetDeviation.sub(a.fracDev)),
      }
    })
  ...
  private calcAssets(
    ...
    const targetSum = assets.reduce(
      (sum, asset) => sum.add(asset.targetValue),
      currentAsset.targetValue
    )
